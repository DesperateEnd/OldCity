 ...
 /* 修改用户个性签名接口 */
 module.exports = updateSignature = function(req,res){
    ...
    if(req.body&&(req.body.signature==''||req.body.signature==undefined||req.body.signature==null)){//没有 图片名
        //没有用户头像
        res.send({
            code:'101',
            data:'',
...
connetion.query('select * from d_user_list where id="'+req.body.uid+'";',function(err,data,filds){ //给他赋值一个叫query的方法，这个方法接受两个参数，第一个参数是你查询数据时候的sql语句，第二个参数是获取查询结果的回调函数。
   ...
   if(data.length==0){//没有查询到
           ...
           return;
            ...
            connetion.query('update d_user_list set signature="'+req.body.signature+'" where id='+req.body.uid+';',function(err1,data1,filds1){
                       if(err1){
                           console.log(err1);  
                           res.send({
                                code:'101',
